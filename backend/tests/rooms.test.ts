  ...
  describe("Admin CRUD & Quy tắc nghiệp vụ (BR06, BR08)", () => {
    ...
    it("MANAGER tạo phòng thành công kèm tiện nghi và ảnh", async () => {
      ...
      const res = await mgrAgent.post("/api/admin/rooms").send({
        ...
        description: "Premium private movie room",
        amenities: ["4K projector", "Dolby Atmos 7.1", "Reclining sofa"],
        images: ["https://example.com/r1.jpg", "https://example.com/r2.jpg"],
      });

      expect(res.status).toBe(201);
      expect(res.body.data.name).toBe("VIP Room 1");
      expect(res.body.data.capacity).toBe(6);
      expect(res.body.data.amenities).toEqual(["4K projector", "Dolby Atmos 7.1", "Reclining sofa"]);
      expect(res.body.data.images.length).toBe(2);

      const overCapacity = await mgrAgent.post("/api/admin/rooms").send({
        name: "Oversized room",
        capacity: 7,
        hourlyPriceVnd: 200_000,
      });
