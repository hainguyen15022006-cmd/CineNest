...
form.addEventListener("submit", async (e) => {
  ...
  await run(async () => {
    ...
    toast("Item added", "success"); await load("");
...
async function load(status: string) {
  ...
  try {
    ...
    list.innerHTML = `<table><thead><tr><th>Order</th><th>Booking</th><th>Item</th><th>Status</th><th></th></tr></thead><tbody>${rows.map((o) => `
      ...
      <td>${o.status === "PENDING" ? `<button type="button" class="small" data-id="${o.id}" data-s="PREPARING">Start preparing</button> <button type="button" class="small secondary" data-id="${o.id}" data-s="CANCELLED">Cancel</button>` : ""}
          ${o.status === "PREPARING" ? `<button type="button" class="small" data-id="${o.id}" data-s="SERVED">Served</button>` : ""}</td></tr>`).join("")}</tbody></table>`;
