...
try {
  ...
  box.innerHTML = `
    <a class="card" href="./schedule.html"><h2>Today's schedule</h2><p class="stat-number">${today.length}</p><p class="muted">${count("CONFIRMED")} waiting · ${count("IN_USE")} in use · ${count("COMPLETED")} completed</p></a>
    <a class="card" href="./schedule.html#overdue"><h2>Overdue bookings requiring action</h2><p class="stat-number${overdue.length ? " error-text" : ""}">${overdue.length}</p><p class="muted">bookings awaiting staff action</p></a>
