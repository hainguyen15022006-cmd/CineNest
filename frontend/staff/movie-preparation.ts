...
await mountLayout("Chuẩn bị phim");
...
async function load() {
  ...
  try {
    ...
    if (!rows.length) return setState(list, "empty", "Không có phim cần chuẩn bị trong ngày này");
    list.innerHTML = `<table><thead><tr><th>Giờ</th><th>Phòng</th><th>Booking</th><th>Phim</th><th>Trạng thái</th><th></th></tr></thead><tbody>${rows
      .map((r) => `<tr><td>${timeVn(r.startAt)}</td><td>${escapeHtml(r.room.name)}</td><td>${escapeHtml(r.code)}</td><td>${escapeHtml(r.movieTitleSnapshot)} (${r.movieDurationSnapshot}′)</td><td>${badge(r.preparationStatus)}</td>
        <td>${r.preparationStatus !== "READY" ? `<button type="button" class="small" data-id="${r.id}" data-v="${r.movieVersion}" data-s="READY">Đã chuẩn bị</button>` : ""}
            <button type="button" class="small secondary" data-id="${r.id}" data-v="${r.movieVersion}" data-s="UNAVAILABLE">Không khả dụng</button></td></tr>`).join("")}</tbody></table>`;
    list.querySelectorAll<HTMLButtonElement>("button[data-id]").forEach((btn) =>
      btn.addEventListener("click", () => run(async () => {
        ...
        toast("Đã cập nhật", "success"); await load();
      }, btn)),
    );
  } catch (e) {
